...
import { Mail, RefreshCw, Loader2, LogIn, X, ExternalLink, ChevronLeft, ChevronRight } from 'lucide-react';
import { fetchUnreadEmails, getAuthUrl, checkAuthStatus, markAsRead, getGmailUrl } from '@/services/gmailService';
...
export function EmailBox() {
  ...
  const handleAuthenticate = async () => {
    try {
      ...
      // Open auth URL in the same window
  ...
  const handleOpenEmail = (emailId) => {
    ...
    // Auto-refresh after a short delay to check if email was read
    setTimeout(() => {
      loadEmails();
    }, 3000);
  };
  ...
  return (
    <Card className="w-full flex flex-col border-2 rounded-xl">
      <CardHeader className="pb-3">
        ...
        <CardDescription>
          Your latest unread messages from Gmail
        </CardDescription>
      </CardHeader>
                    ...
                    <div className="flex items-end justify-between gap-2">
                      ...
                      <div className="flex gap-1 flex-shrink-0">
                        ...
                        <Button
                          ...
                          disabled={dismissingIds.has(email.id)}
                          title="Mark as read"
                        >
                          {dismissingIds.has(email.id) ? (
